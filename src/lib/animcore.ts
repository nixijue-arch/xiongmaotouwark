...
export interface TTSClip extends BaseClip { trackId: 'tts'; text: string; voice: string; audioSrc?: string; audioDuration?: number; genFailed?: boolean; audioEngine?: 'youdao' | 'baidu' | 'proxy' | 'edge'; linkedCaptionId?: string; playbackRate?: number; userAudio?: boolean; /* 用户上传的 mp3 配音 — audioSrc 直接用, 不走 TTS auto-gen */ }
...
export const GIF_PRESETS: GifPreset[] = [
  ...
  { id: 'custom',      label: '自定义尺寸',   width: 480, height: 480, fps: 25, defaultDuration: 5,   maxDuration: 10, note: '自己设画板宽×高 (px)' },
];
